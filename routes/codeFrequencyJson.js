...
exports.code_frequency_plot = function(masterGitLogJson){
  console.log('We are in the converter function');
  ...
  masterGitLogJson.map(function(gitLogObj){
    ...
    //if(gitLogObj.type == "Commit") {
      currObjDate = new Date(gitLogObj.commitDate);
      ...
      if (weekWiseStatsObj[currObjYear] === undefined)
        ...
        if(currObjWeek in weekWiseStatsObj[currObjYear]) {
          weekWiseStatsObj[currObjYear][currObjWeek].insertions += parseInt(gitLogObj.insertion);
          weekWiseStatsObj[currObjYear][currObjWeek].deletions -= parseInt(gitLogObj.deletion);
        }
        else {
          //This week has traversed for the first time in this year
          weekWiseStatsObj[currObjYear][currObjWeek] = {};
          weekWiseStatsObj[currObjYear][currObjWeek].insertions = parseInt(gitLogObj.insertion);
          weekWiseStatsObj[currObjYear][currObjWeek].deletions = parseInt(gitLogObj.deletion)*-1;
  ...
  return weekWiseStatsObj;
  //console.log(JSON.stringify((weekWiseStatsObj), null, '\t').length);
  //return JSON.stringify((weekWiseStatsObj), null, '\t');
}
